...
fun HomeScreen(receipts: List<Receipt>, onAdd: () -> Unit, onOpen: (Long) -> Unit, onSeeAll: () -> Unit) {
    ...
    Scaffold(
        floatingActionButton = {
            FloatingActionButton(onClick = onAdd) { Icon(Icons.Default.Add, contentDescription = "Añadir ticket") }
        }
    ) { padding ->
        LazyColumn(
            ...
            item {
                ...
                Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
                    Column(Modifier.padding(18.dp)) {
                        ...
                        Text("En ${receipts.size} tickets", color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
                Spacer(Modifier.height(8.dp))
            ...
            if (receipts.isEmpty()) {
                ...
                items(receipts.take(5), key = { it.id }) { receipt -> ReceiptRow(receipt, onOpen) }
                item { Button(onClick = onSeeAll, modifier = Modifier.fillMaxWidth()) { Text("Ver todos") } }
            }
...
fun ReceiptListScreen(receipts: List<Receipt>, query: String, onQuery: (String) -> Unit, onOpen: (Long) -> Unit) {
    val filtered = remember(receipts, query) {
        if (query.isBlank()) receipts else receipts.filter { it.merchant.contains(query, true) || it.rawText.contains(query, true) }
    }
    Scaffold(topBar = { SmallTopAppBar(title = { Text("Tickets") }) }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp)) {
            OutlinedTextField(
                ...
                placeholder = { Text("Buscar tickets") }, leadingIcon = { Icon(Icons.Default.Search, null) }, singleLine = true
            )
            Spacer(Modifier.height(12.dp))
            LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                items(filtered, key = { it.id }) { ReceiptRow(it, onOpen) }
            }
        }
    }
}
    ...
    Card(onClick = { onOpen(receipt.id) }, modifier = Modifier.fillMaxWidth()) {
        Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Column(Modifier.padding(start = 14.dp).weight(1f)) {
                ...
                Text(receipt.date.ifBlank { "Fecha no detectada" }, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ReceiptDetailScreen(receipt: Receipt?, onBack: () -> Unit, onDelete: (Receipt) -> Unit) {
    Scaffold(topBar = { SmallTopAppBar(title = { Text("Detalle") }, navigationIcon = { IconButton(onBack) { Text("‹") } }) }) { padding ->
        if (receipt == null) {
            BoxMessage("No se ha encontrado el ticket", Modifier.fillMaxSize().padding(padding))
            return@Scaffold
        }
        LazyColumn(Modifier.fillMaxSize().padding(padding).padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            item {
                Text(receipt.merchant.ifBlank { "Comercio no detectado" }, style = MaterialTheme.typography.headlineMedium)
                Field("Fecha", receipt.date)
                Field("Total", receipt.total?.let(euro::format) ?: "No detectado")
                Field("IVA", receipt.tax?.let(euro::format) ?: "No detectado")
                Field("N.º de ticket", receipt.receiptNumber.ifBlank { "No detectado" })
                Field("Categoría", receipt.category)
                if (receipt.imagePath.isNotBlank()) {
                    val bitmap = remember(receipt.imagePath) { BitmapFactory.decodeFile(receipt.imagePath) }
                    bitmap?.let { androidx.compose.foundation.Image(it.asImageBitmap(), null, Modifier.fillMaxWidth().height(260.dp)) }
                }
...
@Composable private fun BoxMessage(text: String, modifier: Modifier) {
    Column(modifier, horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) { Text(text) }
}
